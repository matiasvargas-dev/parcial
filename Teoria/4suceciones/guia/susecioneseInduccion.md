...
> Nota: Este documento usa notación LaTeX (KaTeX). Para ver las fórmulas correctamente en VS Code instala la extensión "Markdown+Math" o "Markdown Preview Enhanced" y abre la vista previa con `Ctrl+Shift+V`.
...
- $a_1 = 2$, $d = 3$
- $a_5 = 2 + (5-1)\times3 = 2 + 12 = 14$ ✓

**Suma de n términos:**
$$
S_n = \frac{n}{2} \times (a_1 + a_n) \quad \text{o} \quad S_n = \frac{n}{2} \times (2a_1 + (n-1)d)
$$

**Ejemplo:** Suma de los primeros 5 términos: $S_5 = \tfrac{5}{2} \times (2 + 14) = \tfrac{5}{2} \times 16 = 40$
...
**Suma de n términos:**
$$
S_n = a_1 \times \frac{r^n - 1}{r - 1} \qquad \text{si } r \neq 1
$$

**Ejemplo:** Suma de los primeros 5 términos: $S_5 = 3 \times \dfrac{2^5 - 1}{2 - 1} = 3 \times 31 = 93$
...
- ¿Cuál es $a_{10}$?
- **Respuesta esperada:** $d = 6$, $a_{10} = 5 + 9\times6 = 59$
...
- Cálculo: $1.5^{15} \approx 437$ vs $100\times15 = 1500$ (todavía lineal), pero $1.5^{50} \approx 88{,}000$ vs $100\times50 = 5{,}000$
...
$$
a_1 = 2\\
a_n = a_{n-1} + 3 \quad \text{para } n \ge 2
$$
...
**Fórmula cerrada:** $a_n = 2 + 3(n-1) = 3n - 1$
...
$$
b_1 = 5\\
b_n = 2 \times b_{n-1} \quad \text{para } n \ge 2
$$
...
**Fórmula cerrada:** $b_n = 5 \times 2^{(n-1)}$
...
F_1 = 1\\
F_2 = 1\\
F_n = F_{n-1} + F_{n-2} \quad \text{para } n \ge 3
$$
...
F_n = \frac{\varphi^n - \psi^n}{\sqrt{5}}\\
	ext{Donde: }\quad \varphi = \frac{1 + \sqrt{5}}{2} \approx 1.618, \quad \psi = \frac{1 - \sqrt{5}}{2} \approx -0.618
$$

**Verificación:**
- `F_5 = (1.618^5 - (-0.618)^5) / √5 ≈ 5` ✓
...
**Pregunta 2:** Dada `d_1 = 1`, `d_n = 3 × d_{n-1}`
...
**Proposición:** Demostrar que $1 + 2 + 3 + \dots + n = \dfrac{n(n+1)}{2}$ para todo $n \ge 1$

#### **PASO 1: BASE (n = 1)**
$$
	ext{Lado izquierdo: } 1\\
	ext{Lado derecho: } \frac{1(1+1)}{2} = \frac{1(2)}{2} = 1 \checkmark\\
	ext{P(1) es verdadera}
$$

#### **PASO 2: HIPÓTESIS INDUCTIVA**
$$
	ext{Asume que P(k) es verdadera: }\\
1 + 2 + 3 + \dots + k = \frac{k(k+1)}{2}
$$

#### **PASO 3: TESIS - Demuestra P(k+1)**
$$
	ext{Debes demostrar: } 1 + 2 + 3 + \dots + k + (k+1) = \frac{(k+1)(k+2)}{2}\\
	ext{Desarrollo: }\\
1 + 2 + 3 + \dots + k + (k+1) = \left[\frac{k(k+1)}{2}\right] + (k+1) \quad [\text{Usando hipótesis inductiva}]\\
= (k+1)\left[\frac{k}{2} + 1\right] = (k+1)\left[\frac{k + 2}{2}\right] = \frac{(k+1)(k+2)}{2} \checkmark
$$

#### **PASO 4: CONCLUSIÓN**
```
Por el principio de inducción matemática, 
∑(i=1 hasta n) i = n(n+1)/2 es verdadera para todo n ≥ 1
```
...
**Proposición:** Demostrar que `2^n > n^2` para todo n ≥ 5

#### **PASO 1: BASE (n = 5)**
```
Lado izquierdo: 2^5 = 32
Lado derecho: 5^2 = 25
32 > 25 ✓
P(5) es verdadera
```
...
#### **PASO 2: HIPÓTESIS INDUCTIVA**
```
Asume: 2^k > k^2 para algún k ≥ 5
```

#### **PASO 3: TESIS - Demuestra P(k+1)**
```
Debes demostrar: 2^(k+1) > (k+1)^2

Desarrollo:
2^(k+1) = 2 × 2^k
        > 2 × k^2                 [Usando hipótesis inductiva]
        = 2k^2
...
#### **PASO 4: CONCLUSIÓN**
```
Por inducción, 2^n > n^2 para todo n ≥ 5
```
...
**Proposición:** Demostrar que `1 + r + r^2 + ... + r^(n-1) = (r^n - 1)/(r - 1)` para r ≠ 1, n ≥ 1

#### **PASO 1: BASE (n = 1)**
```
Lado izquierdo: 1
Lado derecho: (r^1 - 1)/(r - 1) = (r - 1)/(r - 1) = 1 ✓
```

#### **PASO 2: HIPÓTESIS INDUCTIVA**
```
Asume: ∑(i=0 hasta k-1) r^i = (r^k - 1)/(r - 1)
```

#### **PASO 3: TESIS**
```
Debes demostrar: ∑(i=0 hasta k) r^i = (r^(k+1) - 1)/(r - 1)

Desarrollo:
∑(i=0 hasta k) r^i = [∑(i=0 hasta k-1) r^i] + r^k
                   = (r^k - 1)/(r - 1) + r^k
                   = [(r^k - 1) + r^k(r - 1)]/(r - 1)
                   = [r^k - 1 + r^(k+1) - r^k]/(r - 1)
                   = [r^(k+1) - 1]/(r - 1) ✓
...
#### **PASO 4: CONCLUSIÓN**
```
Por inducción, la fórmula es verdadera para todo n ≥ 1
```
...
**Pregunta 1:** Demuestra por inducción que `1^2 + 2^2 + 3^2 + ... + n^2 = n(n+1)(2n+1)/6`

**Respuesta esperada (esquema):**
```
Base: n=1: 1 = 1(2)(3)/6 = 1 ✓
Paso: Si ∑k^2 = k(k+1)(2k+1)/6, entonces
      ∑(i=1 a k+1) i^2 = [k(k+1)(2k+1)/6] + (k+1)^2
                       = (k+1)[k(2k+1) + 6(k+1)]/6
                       = (k+1)[2k^2 + 7k + 6]/6
                       = (k+1)(k+2)(2k+3)/6 ✓
```
...
Total en árbol de altura h:
T(h) = 1 + 2 + 4 + ... + 2^h = 2^(h+1) - 1
```
...
| O(n²) | Cuadrática | Bubble sort | 100 | 10,000 | 1,000,000 |
| O(2^n) | Exponencial | Fibonacci naive | 1024 | ∞ | ∞ |

#### **¿Por qué importa?**

Una app con O(n²) que maneja 1000 usuarios:
...
Comparaciones totales: ∑(i=1 a n) n = n × n = n²
Complejidad: O(n²)
```
...
**Modelo:** `U_n = 100 × 1.2^(n-1)`

**Proyección:**
- Mes 1: 100 usuarios
- Mes 6: 100 × 1.2^5 ≈ 249 usuarios
- Mes 12: 100 × 1.2^11 ≈ 866 usuarios
- Mes 24: 100 × 1.2^23 ≈ 7,304 usuarios
- Mes 36: 100 × 1.2^35 ≈ 61,917 usuarios ← Exponencial
...
- t = 24 horas: 2^72 ≈ 4.7 × 10²¹ bacterias (¡más que granos de arena!)
...
**Proyección:**
- Año 1: 1000 × (1.00417)^12 ≈ $1051.14
- Año 5: 1000 × (1.00417)^60 ≈ $1283.23
- Año 20: 1000 × (1.00417)^240 ≈ $2693.71
- Año 50: 1000 × (1.00417)^600 ≈ $12,117.77
...
**Fórmula Cerrada:** `H(n) = 2^n - 1`

**Verificación:** H(3) = 2³ - 1 = 7 ✓

**Insight Real:** Con 64 discos, necesitas 2⁶⁴ - 1 = 18 billones de movimientos. A 1 movimiento/segundo, ¡tardaría 585 años!
...
**Pregunta 1:** Un algoritmo procesa datos con complejidad O(n²). Si actualmente procesa 10,000 registros en 1 segundo, ¿cuánto tardará con 100,000 registros?

- **Respuesta esperada:** 
```
O(n²) significa: Tiempo ∝ n²
10,000² = 100,000,000 unidades → 1 segundo
100,000² = 10,000,000,000 unidades
Proporción: 10,000,000,000 / 100,000,000 = 100
Tiempo: 100 segundos
```
...
- **Respuesta esperada:**
```
SA: a_n = a_1 + 1000000(n-1) → crece linealmente
SG: b_n = 1 × 1.01^(n-1) → crece exponencialmente

En algún n, 1.01^n > 1000000×n
Ejemplo: n = 2000
1.01^2000 ≈ 2.7×10^8 vs 2×10^9 (SG gana)
```
...
| SG | `a_n = a_1 × r^(n-1)` | Interés compuesto |
| Suma SA | `S_n = n/2(a_1 + a_n)` | Total acumulado lineal |
| Suma SG | `S_n = a_1(r^n - 1)/(r-1)` | Total acumulado exponencial |
| Fibonacci | `F_n = (φ^n - ψ^n)/√5` | Naturaleza y algoritmos |
